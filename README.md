...
- Service Worker 只缓存应用资源和 `cards.enc.json`；解密后的卡片只存在于当前页面内存。勾选“记住此设备”后，仅将当前版本的不可导出 `CryptoKey` 和公开版本元数据保存到 IndexedDB，不保存密码或解密后的卡片。
- “锁定”会清除内存中的题库并删除本机记忆；题库重新发布后盐值变化，通常需要重新输入一次密码。
